...
const STOCK_QUOTE_CACHE_TTL_MS = 5 * 60 * 1000;
...
function isProxyTradeInconsistent({
  tradePrice,
  seriesLast,
  dayClose,
}: {
  tradePrice: number | null;
  seriesLast: number | null;
  dayClose: number | null;
}) {
  if (tradePrice == null) return false;

  const reference = seriesLast ?? dayClose;

  if (reference == null || reference <= 0) return false;

  const deviation = Math.abs(tradePrice - reference) / reference;
...
async function fetchStockQuote(ticker: string): Promise<{
  ...
  try {
    ...
    const tradePrice = toPositiveNumber(snapshot?.lastTrade?.p);
    const dayClose = toPositiveNumber(snapshot?.day?.c);
...
function isSuspiciousStockQuote(
  quote: {
    price: number | null;
    prevClose: number | null;
  } | null
...
function rememberGoodStockQuote(ticker: string, quote: StableStockQuote | null) {
  if (!quote || isSuspiciousStockQuote(quote)) return;

  lastGoodStockQuoteCache.set(ticker, {
    ...quote,
    cachedAt: Date.now(),
  });
}

function getCachedGoodStockQuote(ticker: string): StableStockQuote | null {
  const cached = lastGoodStockQuoteCache.get(ticker);

  if (!cached) return null;
  if (Date.now() - cached.cachedAt > STOCK_QUOTE_CACHE_TTL_MS) {
    lastGoodStockQuoteCache.delete(ticker);
    return null;
  }
  ...
  return {
    ...
    updatedMs: cached.updatedMs,
  };
}

async function fetchStableStockQuote(ticker: string, attempts = 3) {
  let lastQuote: Awaited<ReturnType<typeof fetchStockQuote>> = null;

  for (let index = 0; index < attempts; index += 1) {
    const nextQuote = await fetchStockQuote(ticker);
    lastQuote = nextQuote;
...
async function fetchMassiveIndexIntradaySeries(indexTicker: string): Promise<number[]> {
  ...
  try {
    ...
    const url =
      ...
      `?sort=desc&limit=120&apiKey=${API_KEY}`;
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (price == null && isHeadlineIndexRequest && proxyTicker) {
      ...
      const proxySeries = await fetchStockIntradaySeries(proxyTicker);
      const proxySeriesLast = proxySeries.at(-1) ?? null;
      ...
      if (proxyQuote) {
        ...
        if (
          isProxyTradeInconsistent({
            tradePrice: proxyQuote.price,
            seriesLast: proxySeriesLast,
            dayClose: proxySeriesLast,
          })
        ) {
          price = proxySeriesLast;
        }
      }
    }
