...
function TopMoveStrip({
  ...
  const stockHref = `/experts/${slug}/stocks/${row.ticker}`;

  const hrefWithSource = analystName?.trim().length
    ? `${stockHref}?analyst=${encodeURIComponent(analystName)}`
    : stockHref;
...
function CoverageAccordion({
  ...
  function buildStockHref(ticker: string) {
    const baseHref = `/experts/${slug}/stocks/${ticker}`;

    if (!analystName?.trim().length) return baseHref;

    return `${baseHref}?analyst=${encodeURIComponent(analystName)}`;
  }

  function analyzeWithSigi(ticker: string) {
    const normalized = normalizeTicker(ticker);
    router.push(`/today?ticker=${encodeURIComponent(normalized)}`);
  }
  ...
  return (
    <div className="mt-4 space-y-3">
      <TopMoveStrip
        ...
        onOpen={(ticker) => setOpenTicker(ticker)}
      />
          ...
          >
            <div className="p-4">
              <div className="flex items-start gap-3">
                <button
                  type="button"
                  onClick={() => router.push(buildStockHref(row.ticker))}
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => analyzeWithSigi(row.ticker)}
                  className="rounded-full border border-cyan-400/25 bg-cyan-400/12 px-4 py-2 text-xs font-semibold text-cyan-200 transition hover:border-cyan-300/40 hover:bg-cyan-400/18"
                >
                  Analyze with Sigi
                </button>
                <button
