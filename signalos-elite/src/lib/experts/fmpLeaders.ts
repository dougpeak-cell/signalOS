...
export const EXPERT_SECTOR_BUCKETS = [
  "Technology",
  "Healthcare",
  "Financial Services",
  "Industrials",
...
export type FmpExpertsFeed = {
  source: string;
  rows: FmpExpertPickRow[];
...
async function getFallbackSectorCandidates() {
  return getSectorCandidates(EXPERT_SECTOR_BUCKETS);
}
  ...
  return Promise.all(
    candidates.map(async (candidate) => {
      ...
      return {
        ...
        upsidePercent,
        score,
      } satisfies FmpExpertPickRow;
    })
  );
}

function addQualifiedRows(bySector: Map<string, FmpExpertPickRow[]>, rows: FmpExpertPickRow[]) {
...
export async function loadFmpExpertsFeed(): Promise<FmpExpertsFeed> {
  if (!FMP_API_KEY) {
  ...
  const bySector = new Map<string, FmpExpertPickRow[]>();
  addQualifiedRows(bySector, enriched);

  const missingSectors = EXPERT_SECTOR_BUCKETS.filter(
    (sector) => (bySector.get(sector) ?? []).length === 0
  );
  ...
  const sectorRows = Object.fromEntries(
    EXPERT_SECTOR_BUCKETS.map((sector) => [
      sector,
      [...(bySector.get(sector) ?? [])].sort(compareRankedRows).slice(0, 10),
    ])
  );

  const diversified: FmpExpertPickRow[] = [];
  ...
  return {
    source:
      strictRows.length > 0
        ? "fmp_diversified_analyst_picks"
        : "fmp_latest_analyst_picks_fallback",
    rows,
    sectorRows,
  };
}