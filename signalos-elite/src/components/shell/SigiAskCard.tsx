    ...
    <section className="glow-panel rounded-3xl p-5">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <button
          ...
          onClick={onExplain}
          disabled={loading}
          className="rounded-xl border border-cyan-400/25 bg-cyan-400/10 px-3 py-2 text-[11px] font-medium text-cyan-100 transition hover:bg-cyan-400/15 disabled:cursor-not-allowed disabled:opacity-50"
        >
          Explain Setup
        </button>
        ...
        <button
          ...
          onClick={onKeyLevels}
          disabled={loading}
          className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-[11px] font-medium text-white/80 transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-50"
        >
          Key Levels
        </button>
        ...
        <button
          ...
          onClick={onRiskView}
          disabled={loading}
          className="rounded-xl border border-rose-400/25 bg-rose-400/10 px-3 py-2 text-[11px] font-medium text-rose-100 transition hover:bg-rose-400/15 disabled:cursor-not-allowed disabled:opacity-50"
        >
          Risk View
        </button>
      </div>
      ...
      {loading ? (
        <div className="mt-5 rounded-2xl border border-cyan-400/15 bg-cyan-400/5 px-4 py-3 text-sm text-cyan-100/80">
      ...
      {response && showResponseCards ? (
        <div className="mt-5">
          <SigiResponseCards response={response} />
        </div>
      ) : null}

      {streaming || bullets.length > 0 || narrative ? (
        <div className="mt-5 border-t border-white/10 pt-5">
          <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
            ...
            {streaming ? (
              <div className="mt-3 rounded-2xl border border-cyan-400/15 bg-cyan-400/5 px-4 py-3 text-sm text-cyan-100/80">
                Sigi is streaming the explanation...
              </div>
            ) : null}
            ...
            {bullets.length > 0 ? (
              <div className="mt-3 space-y-2">
                {bullets.map((bullet) => (
                  <div
                    key={bullet}
                    className="rounded-xl border border-white/8 bg-white/5 px-3 py-2 text-sm text-white/78"
                  >
                    • {renderTickerText(bullet)}
                  </div>
                ))}
              </div>
            ) : null}
            ...
            {narrative ? (
              <div className="mt-4 rounded-2xl border border-white/8 bg-white/4 px-4 py-3 text-sm leading-6 text-white/80">
                {renderTickerText(narrative)}
              </div>
            ) : null}
          </div>
        </div>
