...
function injectTicker(headline: string, ticker?: string) {
  if (!ticker) return headline;

  return headline.replace(/Markets/i, ticker);
}
...
export default function SigiNewsCard({ items, activeTicker }: SigiNewsCardProps) {
  return (
      ...
      <div className="mt-4 space-y-3">
        {items.length === 0 ? (
          ...
          items.map((item, index) => {
                  ...
                  <div className="min-w-0 flex-1">
                    {activeTicker ? (
                      <div className="text-[11px] uppercase tracking-[0.2em] text-cyan-300/70">
                        In focus: {activeTicker}
                      </div>
                    ) : null}

                    <div className="text-sm font-medium leading-6 text-white">
                      {injectTicker(item.headline, activeTicker)}
                    </div>
            ...
            return <div key={`${item.href ?? item.id}-${index}`}>{content}</div>;
          })
        )}
      </div>
    </section>
