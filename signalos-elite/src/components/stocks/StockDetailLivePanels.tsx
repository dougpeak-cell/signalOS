...
export default function StockDetailLivePanels({
  ...
  const exchangeBadge = [companyProfile?.exchange, companyProfile?.currency]
    ...
    .join(" • ");
  const websiteLabel = `Visit ${companyProfile?.name ?? liveTicker}`;
  const heroMetrics = [
    analysisPrice != null
      ? {
          key: "price",
                ...
                {companyProfile?.weburl ? (
                  <>
                    <a
                      ...
                      className="inline-flex w-full items-center justify-center gap-3 rounded-full border border-cyan-400/25 bg-cyan-400/10 px-4 py-2 text-sm font-semibold text-cyan-200 transition hover:bg-cyan-400/20 md:hidden"
                    >
                      {companyProfile.logo ? (
                        <img
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="hidden items-center gap-3 rounded-full border border-cyan-400/25 bg-cyan-400/10 px-4 py-2 text-sm font-semibold text-cyan-200 transition hover:bg-cyan-400/20 md:inline-flex"
                    >
                      {companyProfile.logo ? (
                        <img
                          src={companyProfile.logo}
