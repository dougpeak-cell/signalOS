...
export default function TickerHover({
  ...
  const { ticker: panelTicker, openPanel, closePanel } = useSigiPanel();
  ...
  const scheduleOpen = () => {
    ...
    openTimerRef.current = window.setTimeout(() => {
      if (panelTicker && normalizeTicker(panelTicker) === normalizedTicker) {
        closePanel()
      }
      setOpen(true);
  ...
  return (
    ...
    >
      ...
      {open ? (
        <span
          className="fixed bottom-20 right-6 z-50 w-85 animate-in fade-in zoom-in-95 duration-150"
          onMouseEnter={scheduleOpen}
          onMouseLeave={scheduleClose}
        >
          <span
